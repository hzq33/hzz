...
_TOOL_AWARE_PROMPT = """
## 工具
真实世界问题（天气、新闻、时间、计算）可以调用工具。
原著情节、人设、关系、外貌不要用网页搜索补全，只用「原著参考」。

调用工具后：
1. 用角色口吻自然转述，不要说「搜索 / 查资料 / 根据结果」。
2. 若内容是角色不可能知道的（不在场、他人私事、尚未经历的情节），即使查到了也要以角色身份表示不清楚，不要剧透。
"""
...
你不是角色，不要扮演。只输出 JSON，不要 markdown 代码块或其他文字：
{{
...
- kept_snippets 只保留与问题直接相关的序号
- answerable=false 时说明缺什么信息，不要补全原文
"""
...
class ImpersonationChatMixin:
    ...
    _FACT_GROUNDING_HINT = (
        "设定冲突时以「原著参考」为准；参考未写明的外貌、关系、经历请说不确定，禁止编造。\n"
        "知识边界：你只知道自己亲身经历或听闻过的事。"
        "工具或检索得到的内容，若发生在你不在场、属别人私事、或你不可能知道的时间地点，"
        "即使查到了也要以角色身份表示不清楚，不要剧透。"
    )
    _NO_FACT_HINT = (
        "## 注意\n"
        "本次未检索到可靠原著事实。涉及设定、外貌、关系时请说不确定，"
        "禁止编造原文未写明的细节。"
    )
    ...
    async def _prepare_final_messages(
        ...
        if tool_facts:
            ...
            final_messages.append({
                "role": "system",
                "content": (
                    "你刚才通过工具获得了以下事实。用角色口吻自然融入回复；"
                    "角色不可能知道的内容不要说出来，也不要提及查资料或工具：\n"
                    + facts_blob
                ),
            })
        else:
            final_messages.append({
                "role": "system",
                "content": "现在以角色身份回复用户。不要复述规则，不要提及工具或检索。",
            })
